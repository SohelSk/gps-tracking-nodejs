/* Original code: https://github.com/cnberg/gps-tracking-nodejs/blob/master/lib/adapters/ET300.js */
f = require('../functions');
crc = require('crc');

exports.protocol = 'ET300';
exports.model_name = 'ET300';
exports.compatible_hardware = ['ET300/supplier'];
...
var adapter = function (device) {
  ...
  this.parse_data = function (data) {
    ...
    if (parts['start'] == '7878') {
      ...
      parts['serial'] = data.substr(data.length - 12, 4);
  ...
  this.authorize = function (msg_parts) {
    ...
    //return ;
    const length = '05';
    const protocal_id = msg_parts.protocal_id;
    const serial = msg_parts.serial;
    const info = msg_parts.device_id;
    var str = length + protocal_id  + serial;

    var crcResult = f.crc16(str);
    var buff = new Buffer.from('7878' + str + crcResult + '0d0a', 'hex');
    // var buff = new Buffer('787805010001d9dc0d0a', 'hex');
    //发送原始数据
    this.device.send(buff);
  };
  this.accept_heartbeat = function (msg_parts){
    const length = '05';
    const protocol_id = msg_parts.data.substr(6, 2);
    const serial = msg_parts.data.substr(msg_parts.data.length - 12, 4);
