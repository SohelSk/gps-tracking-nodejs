...
function Device(adapter, connection, gpsServer) {
  ...
  this.heartbeat = function (msgParts) {
    _this.do_log('This is a Heartbeat');
    _this.emit('heartbeat', this.getUID(), msgParts);
  };
  this.accept_heartbeat = function (msgParts) {
    this.adapter.accept_heartbeat(msgParts);
  }
